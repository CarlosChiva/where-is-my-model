...
/**
 * AddPcModal — Modal dialog for adding a new GPU server.
 *
 * Props:
 *   onSave  — (data: { nombre: string, ip: string, gpus: Array<{name:string,vram:number}> }) => void
 *   onClose — () => void
 */
export default function AddPcModal({ onSave, onClose, loading = false, error = null, clearError }) {
  const [formData, setFormData] = useState({
    nombre: '',
    ip: '',
    gpus: [{ name: 'GPU 1', vram: '' }],
  });
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold text-text-primary mb-6">Add New Server</h2>

...
/**
 * EditServiceModal — Modal dialog for editing an existing service on a GPU server.
 *
 * Pre-fills form fields from the `service` prop including current assignedGpu.
 * Validates against per-GPU capacity by excluding the service being edited
 * from the services array, ensuring both same-GPU VRAM changes and cross-GPU
 * reassignment are checked correctly (the freed allocation on source GPU is
 * accounted for automatically).
 *
 * Props:
 *   pcId            — Server ID (MongoDB _id)
 *   serviceIndex    — Array index of the service being edited
 *   service         — Existing service object { nombre, puerto, gpu, assignedGpu }
 *   pcGpus          — PC's gpus array [{ name: string, vram: number }]
 *   pcServices      — Current services array on this PC (full, including this one)
 *   onSave          — (data: { pcId, index, nombre, puerto, gpu, assignedGpu }) => void
 *   onCancel        — () => void
 ...
 */
export default function EditServiceModal({ pcId, serviceIndex, service, pcGpus, pcServices, onSave, onCancel, loading = false, error = null, clearError }) {
  /* ── Pre-populate form data from the incoming service object ─── */
  const [formData, setFormData] = useState({
    nombre:       service?.nombre ?? '',
    puerto:       String(service?.puerto ?? ''),
    gpu:          String(service?.gpu ?? ''),
    assignedGpu:  String(service?.assignedGpu ?? '0'),
  });

  const [errors, setErrors] = useState({});
  const [touched, setTouched] = useState({ nombre: false, puerto: false, gpu: false, assignedGpu: false });

  /* Capture services array minus the one being edited.
   * This is critical for per-GPU capacity validation: removing this service's
   * prior allocation from its GPU so the validator can correctly assess both
   * same-GPU VRAM changes and cross-GPU reassignments. */
  const pcServicesExSelf = pcServices.filter((_, i) => i !== serviceIndex);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
       ...
       * Pass filtered services so this service's current allocation
       * is excluded from the remaining-VRAM computation. */
      const { errors: allErrors } = validateServiceForm(updatedData, pcServicesExSelf, pcGpus);
      setErrors(allErrors);
      return updatedData;
    });
  ...
  const handleSubmit = (e) => {
    ...
    const finalResult = validateServiceForm(formData, pcServicesExSelf, pcGpus);
    setErrors(finalResult.errors);

    if (!finalResult.valid) return;

    /* Sanitise and dispatch upward — include assignedGpu for backend schema compliance */
    onSave({
      pcId:         pcId,
      index:        serviceIndex,
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold text-text-primary mb-6">Edit Service</h2>
        ...
        <div className="mb-4">
          ...
          {(!Array.isArray(pcGpus) || pcGpus.length === 0) ? (
            ...
            <>
              ...
              >
                {pcGpus.map((gpu, idx) => {
                  const remain = getRemainingVram(pcGpus, pcServicesExSelf, idx);
                  return (
